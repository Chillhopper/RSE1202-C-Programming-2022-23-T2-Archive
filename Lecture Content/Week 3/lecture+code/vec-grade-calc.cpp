...
This program read a student's midterm, final exam, and an unknown number of assignment grades
and then computes that student's course grade which is then printed to standard output.
Assume that the student's final exam counts for 40% of the final grade, the
midterm exam counts for 20%, and the median homework grade makes up the
remaining 40%.
The program exhibits the use of the string, vector, use of algorithm (sort),
