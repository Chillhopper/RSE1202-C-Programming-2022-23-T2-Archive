...
// from the standard input stream using the overloaded operator>> function
// provided by the interface of std::string.
// Notice that similar to scanf(), a string is delimited by
// whitespace characters. This means the following program cannot be
// used to read the full name of the user.
/*
The string type is defined in the standard header <string>. An object of
type string contains a sequence of zero or more characters. If n is an
integer, c is a char, is is an input stream, and os is an output stream,
then the string operations include:
